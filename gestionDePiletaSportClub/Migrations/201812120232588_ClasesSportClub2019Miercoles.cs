namespace gestionDePiletaSportClub.Migrations
{
    ...
    public partial class ClasesSportClub2019Miercoles : DbMigration
    {
        public override void Up()
        {
            DateTime wed9hs = new DateTime(2019, 1, 2, 9, 0, 0);
            DateTime wed20hs = new DateTime(2019, 1, 2, 20, 0, 0);
            DateTime wed17hs = new DateTime(2019, 1, 2, 17, 0, 0);
            DateTime wed19hs = new DateTime(2019, 1, 2, 19, 0, 0);
            int id = 1;
            ...
            for (int i = 0; i < 52; i++)
            {
                ...
                id++;
                Sql(String.Format("Insert into Actividad ( levelid,tipoActividadid,amountofenrollment,pendingenrollment,schedule,estadoActividadid,membershiptypeid) values ({0},{1},8,8,'{2}',1,2)", 2, 1, wed19hs.ToString("yyyy-MM-ddTHH:mm:ss")));
                id++;
                Sql(String.Format("Insert into Actividad ( levelid,tipoActividadid,amountofenrollment,pendingenrollment,schedule,estadoActividadid,membershiptypeid) values ({0},{1},8,8,'{2}',1,2)", 3, 1, wed19hs.ToString("yyyy-MM-ddTHH:mm:ss")));
                id++;
                Sql(String.Format("Insert into Actividad ( levelid,tipoActividadid,amountofenrollment,pendingenrollment,schedule,estadoActividadid,membershiptypeid) values ({0},{1},8,8,'{2}',1,2)", 5, 1, wed19hs.ToString("yyyy-MM-ddTHH:mm:ss")));
                id++;
                #endregion
